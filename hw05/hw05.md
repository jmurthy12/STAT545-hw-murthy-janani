...
Match country in gapminder with country in the new data frame :country\_presidents

``` r
country_presidents_level <- levels(country_presidents$country)

gapWithMatchingcountries <- gapminder %>% 
  filter(year == 2005,country %in% country_presidents$country) %>% 
  mutate(country = fct_relevel(country, country_presidents_level)) %>% 
  droplevels()
```

They are my rough solution to fix the problem of matching the same variable with different level orders in different datasets. First I define a variable I call p2 and to it I associate the sports of all the countries in both datasets.

``` r
gapminder_country_levels <- levels(gapWithMatchingcountries$country)
newSetCountryPresidents <- country_presidents %>% 
  filter(country %in% gapminder_country_levels) %>% 
  droplevels()
```
...
``` r
gapcountriesPresidents <- gapWithMatchingcountries %>% 
  mutate(country = as_factor(levels(newSetCountryPresidents$country)), presidents = as_factor(levels(newSetCountryPresidents$presidents))) %>% 
  select(country, continent, presidents, lifeExp, pop, gdpPercap)

(gapcountriesPresidents)
```

    ## # A tibble: 0 x 6
    ## # ... with 6 variables: country <fctr>, continent <fctr>,
    ## #   presidents <fctr>, lifeExp <dbl>, pop <int>, gdpPercap <dbl>
